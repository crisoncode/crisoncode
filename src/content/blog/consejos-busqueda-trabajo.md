---
author: Cristian Estarlich
pubDatetime: 2025-12-08T15:00:00Z
title: Consejos para procesos de búsqueda de trabajo
postSlug: consejos-para-procesos-de-busqueda-de-trabajo
featured: false
draft: false
tags:
  - Carrera profesional
  - Desarrollo profesional
  - Búsqueda de trabajo
description: Consejos prácticos y experiencias personales sobre cómo afrontar los procesos de búsqueda de trabajo en el sector de la programación en España.
---

# Consejos para procesos de búsqueda de trabajo

Antes de empezar con las secciones que tengo en la cabeza me gustaría decir que tienes que adaptarte a ellas a tu manera, cada persona es un mundo y se siente de una forma u otra cuando trabaja, esto tienes que tomártelo también como tal, buscar trabajo, en sí es como trabajar, deberás invertir X horas al día en buscar y organizarte, en entrenar tus skills para las pruebas técnicas, etc... Aquí a diferencia de trabajando como tal no hay normas, por lo tanto la disciplina es importantísima, no puedes parar porque en la primera te rechazan o en la tercera no te contestan, es un proceso hasta llegar a tu objetivo y día a día tienes que esforzarte.

> Te aconsejo que te montes una base de datos sencilla en notion sobre donde has hecho apply, que evalues las empresas de forma numérica (puntuando entre 0 y 5 por ejemplo) y decidas donde invertir más esfuerzos.

# 🏗️ Como encontrar empresas

✅ No te cierres a un solo canal

Hay muchos canales y comunidades donde puedes encontrar trabajo. Te dejo aqui mis favoritas:

## Barcelona engineers

Es una comunidad de slack de Barcelona
[https://slack.bcneng.org](https://slack.bcneng.org/)

En esta comunidad hay un canal que se llama Hiring Job Board, donde puedes ver ofertas detalladas.

![image.png](/assets/consejos-busqueda-trabajo-1.png)

Este slack desde mi punto de vista te da la posibilidad de entablar una conversación con la persona al cargo del proceso o alguien cercano al mismo. Esto es una ventaja competitiva si sabes como hacerlo. Puedes hacer alguna pregunta a esa persona (etiquetada al final), alguans preguntas que se me ocurren son:

> Hola Radu, como funciona el proceso de entrevista, que fases hay?

> Hola María, he visto que en la oferta pone que trabajáis con Postgres, es un requerimiento 100% obligatorio?

No os sintáis cohibidos por hablarle a esa persona, dado que es justamente lo que está esperando publicando esto en Barcelona Engineers.

### Linkedin

En LinkedIn la competencia es mucho más feroz y son canales que ahora mismo están masificados tanto de gente buscando ofertas como de gente buscando trabajo.

Os puedo dar algun tip sobre esto:

‼️ Al igual que el curriculum, tu titulo y descripcion son importantisimos

❗ Si una oferta os llama y veis que hay 100 personas inscritas ( o 10 mil, me da igual) no os desaniméis, es bastante común que la gente aplique a puestos en los que no encaja en absoluto, por ejemplo gente de india aplicando a un trabajo full office en Barcelona, o un ingeniero quimico aplicando a un puesto de frontend developer (yo tampoco entiendo porque la gente lo hace pero es 100% real).

❓ Si os contacta algún recruiter tratad de sacar el máximo de información posible antes de dar el teléfono, me he visto mas de una vez recruiters que tratan de contactarme para posiciones en las que no encajo.

ℹ️ Mantened el LinkedIn lo más actualizado posible

### InfoJobs

Si, no estoy de broma, infojobs con los años ha mejorado mucho y ya no es un portal raro con ofertas dudosas.

### Joppy

Funciona bastante bien, tiene un modo en el cual creas tu perfil y te relacionan con ofertas al más puro estilo tinder

# 🫂 Networking

Tratad de hablar con gente con la que habéis trabajado o habéis tenido algún tipo de relación laboral, esto suele funcionar bastante bien, los referals son una de las formas más rápidas de encontrar trabajo.

En estos casos también depende de como has cuidado tus amistades laborales, si no has vuelto a hablar con amigos de tu anterior trabajo en los últimos 3 años igual queda un poco raro.

## Tips sobre el Curriculum

<div class="md:grid-cols-2 md:items-start mb-20 grid grid-cols-2 items-center gap-10 columns-2 mb-16">

<div>
<img src="/assets/consejos-busqueda-trabajo-2.png" alt="Ejemplo de CV con zonas roja y naranja marcadas" class="h-full" />
</div>

<div >

<span style="color: red; font-weight: bold;">Zona roja</span>

Esta zona es variable, es tu título, como te van a identificar los recruiters o HR, si aplicas a una oferta para fullstack, no pongas que eres software engineer o backend developer, pon en el título que eres fullstack developer, por lo contrario si en la siguiente, es backend developer, pon que eres backend developer.

_Obviamente no mientas, si no sabes de backend no lo pongas._

<span style="color: orange; font-weight: bold;">Zona naranja</span>

Lo mismo, zona variable, si vas a aplicar a una empresa de PHP y React, no pongas primero tu experiencia en Node por más años que tengas en ella, intenta adaptar un poco tu CV a lo que ellos piden sin llegar a mentir.

Debéis entender que el Curriculum es solo un papel para aplicar un filtro previo, si tu te ves con skills para afrontar ese trabajo, lo que tienes que hacer es usar el documento para pasar el filtrado y luego en las fases de la entrevista ya podrás demostrar tus skills.

</div>

</div>

# Tips cuando ya te han contactado para hacer entrevista.

Normalmente las entrevistas se dividen de esta forma:

## 🗣️ **Entrevista con HR - Cultural fit.**

Aqui ven que te adaptas a como es la empresa, te preguntan por tu experiencia, que has hecho en otras empresas, que objetivos tienes, etc…

Aquí mis consejos son:

✔️ Preparate un listado de goals de cosas que hayas hecho de las cuales te sientas orgullos@ y cuando digo preparar es que los escribas en algún sitio, para que cuando te toque explicarlo lo tengas memorizado y lo expliques transmitiendo seguridad, es vital ensayar un poquito estas cosas, a veces los nervios o hacer las entrevistas en un idioma que no es el nuestro nativo, nos juegan malas pasadas y ten claro una cosa: transmitir seguridad es vital.

✔️ Si no se te ocurre nada, habla con tus compañeros de trabajo, igual ellos te saben decir, ostras pues mira esto que hiciste nos encantó y tu quizás ni lo valoras.

Estas fases suelen ser bastante fáciles de superar, a veces te preguntan por hobbies, o que describas alguna situación rara o mala y como saliste de ella, pero en general no son muy duras.

> **💡❕Tip:**
> Estudia la empresa para la que vas a empezar el proceso, les dices que has mirado la web, que has visto sus videos en youtube (obviamente miralo, no te inventes las cosas) e incluso de lo que has visto apuntate alguna duda, eso a HR le va a encantar, porque veran ya una proactividad muy buena de primeras.

## 🧪 Entrevista técnica

Aqui es donde viene la salsa, yo he visto de todos los colores:

- A veces hay de pair programming
- A veces hay que son "deberes para casa", sea como sea aqui van mis consejos.

Voy a dividir esta parte en 2, primero la charla o entrevista y luego la prueba técnica:

### Charla:

Estudiate muy bien su stack tecnológico, en la oferta a la que has hecho apply debería estar:

Esto te permite primero sacar conversaciones chulas en las entrevistas, por ejemplo si pone que usan react pero no usan Next, les dices, que tal os va con React Router? a nostros nos dio X problemas.

Si por otro lado ellos usan una tecnología y tu usas otra, por ejemplo tu usas RabbitMQ y ellos Kafka pues te puedes estudiar los pros y cons de las dos y comentarles tu experiencia con colas y que has visto que con Kafka igual algun problema tuyo en rabbit ya lo tiene resuelto, al final se trata de que tu puedas demostrar que técnicamente tienes expertise en algunos campos y que ellos encima vean que has estudiado su stack y lo has comparado con el tuyo, estas cosas te haran marcar un extra por encima de otros candidatos.

Yo suelo tener preparados dos goals técnicos impuestos por el trabajo del dia a dia.

✔️ Esta tarea la resolví así, usando este patrón o haciendo esto otro

✔️ En esta otra tome esta decisión que me llevo a X.

Otro goal pero este que haya salido de mi, sin petición explicita de nadie.

✔️ En un punto vi que esto no funcionaba bien y pensé como arreglarlo o como construirlo, diseñé un plan lo presenté y voilá.

### Prueba técnica

Normalmente te mandan un repo de github con un _tiny-project_ y un readme de lo que tienes que hacer.

1. El mínimo que se espera:
   1. Que funcione, parece estupido, pero a veces la gente entrega cosas que no funcionan.
   2. Que haya testing, estamos en 2025, como no testees tu código es pa darte dos collejas.
   3. Intenta iterar, si ves algo que no te gusta como ellos te lo han dado pues cambialo, a veces son quick wins, ejemplos:
      1. He visto que el vite que compila react y que tenéis en el repo no esta actualizado, lo he actualizado por X motivo. (Son 3 minutos maximo hacer eso)
      2. Los endpoints que me habéis pasado estaban en MVC, yo me siento más comodo en DDD así que he aplicado la arquitectura y aparte he añadido el patrón value object.

      Se me ocurren mil ideas, aqui se trata de demostrar que tu no sacas tareas y punto, si no que vas mas allá y que le das al coco.

   4. El readme o documento donde explicas que has hecho (puedes crear un documento rollo, [task.md](http://task.md) y allí lo metes todo, da igual el formato, aunque markdown suele ser lo normal)
      1. Esto aunque parezca una tontería es **SUPER MEGA IMPORTANTE,** en este documento tienes voz para explicar porque has hecho X o Y, que decisiones has tomado, lo que decía de iterar en el punto 1, aqui lo tienes que escribir, porque has iterado y has mejorado algo, cualquier decision o cosa que hayas visto escribela, e incluso aunque no lo hayas hecho puedes decir algo tipo: "he visto que en el front tenéis un poco de caos en el CSS; no he llegado a tocarlo porque no lo veía una prioridad, pero esta claro que se puede mejorar"
      2. Si no tienes muy claro como estructurar el doc, tira de AI para montarlo, te ayudara mucho.

2. Uso de inteligencia artificial en las pruebas
   1. Desde mi punto de vista, se puede usar sin problemas, pero no hagamos el paleto, por ejemplo:
      1. La AI tiene por costumbre comentar todo el código, borra los comentarios o pegará el canteo (pídele que los borre o apoyate en [agents.md](http://agents.md) con rules para no tener ni que pedirlo)
      2. En general haz bastante code review del código que genera para que tenga sentido y no se note demasiado.
      3. Yo por ejemplo en alguna prueba he sido super sincero y lo he puesto en el markdown, la parte del front la he hecho 100% con AI dado que mi prueba esta focalizada al backend no he querido invertir tiempo en front. Obviamente esta testeado que todo funciona pero no he ido a mejorarlo

## 💰Negociación

Esta es la parte que a todos nos interesa, saber los benefits y que salario vamos a ganar.

Yo os propongo que penséis en un rengo de cifras antes de empezar las entrevistas, acordaos que la mejor forma de mejorar vuestro salario es cambiar de empresa a costa de asumir cierto riesgo y salir de la zona de comfort.

No tiene sentido que hagáis procesos sin tener claro los beneficios y salario dado que igual perdéis el tiempo, cierto es que esto aplica cuando tienes mas seniority, es triste pero es así.

Tips para negociar:

✅ Si tu ahora mismo estas en 40mil€/brutos anuales y sabes que tienes una subida este año de 4k, puedes ya directamente añadirlos si te preguntan aunque no hayas percibido este augmento. Esta claro que para cambiar vas a querer más de 44mil entonces.

✅ En mi caso cuando me preguntan que expectativas salariales tengo, digo lo que cobro y a partir de alli que ellos decidan.

✅ No dudes en preguntar todo lo relativo a esto, sera tu mejor oportunidad para hacerlo, por ejemplo si te dicen que el trabajo es 100% remoto, pregunta si esta escrito por contrato o puede cambiar en un futuro. Lo mismo para seguros médicos, etc..
